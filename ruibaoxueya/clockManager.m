...
-(BOOL) createDb{
    ...
    {
        char* errorMsg;
        const char *createSql=" create table if not exists u_colcokInfo (Id integer,mUserCode text,mUserName text,mUserPwd text,mUserSex integer,mUserAge integer,mUserKg integer, mUserHigh integer,primary key('Id'));";
...
-(BOOL)insert:(clockInfo*)info
{
    ...
    if(path!=Nil&&info!=Nil)
    {
        ...
        if(result==SQLITE_OK)
        {
            const char* insertSql=" insert or replace into u_colcokInfo(Id ,mUserCode ,mUserName ,mUserPwd ,mUserSex ,mUserAge ,mUserKg , mUserHigh ) values (?,?,?,?,?,?,?,?);";
            sqlite3_stmt *stmt;
...
-(BOOL)update:(clockInfo*)info
{
    ...
    if(path!=Nil&&info!=Nil)
    {
        ...
        if(result==SQLITE_OK)
        {
            const char* insertSql=" update u_colcokInfo set  mUserCode=? ,mUserName=? ,mUserPwd=? ,mUserSex=? ,mUserAge=? ,mUserKg=? , mUserHigh=?  where Id=? ;";
            sqlite3_stmt *stmt;
            if(sqlite3_prepare_v2(database, insertSql, -1, &stmt, nil)==SQLITE_OK)
            {
...
-(BOOL)deleteById:(int)mid
{
    ...
    if(path!=Nil&&mid>0)
    {
        ...
        if(result==SQLITE_OK)
        {
            const char* insertSql=" delete from  u_colcokInfo where Id=?;";
            sqlite3_stmt *stmt;
            if(sqlite3_prepare_v2(database, insertSql, -1, &stmt, nil)==SQLITE_OK)
            {
...
-(BOOL)deleteAll
{
    ...
    {
        char* errorMsg;
        const char *deleteSql=" delete from u_colcokInfo where Id>0;";
...
-(clockInfo*)getInfo:(sqlite3_stmt *)statement
{
    
    char *remark=(char*)sqlite3_column_text(statement, 1);
    clockInfo *info=[[clockInfo alloc] init];
...
-(NSMutableArray*)queryInfo
{
    ...
    if (path!=Nil) {
        ...
        if(result==SQLITE_OK)
        {
            NSString* selectStr=[[NSString alloc]initWithFormat:@"select Id ,mUserCode ,mUserName ,mUserPwd ,mUserSex ,mUserAge ,mUserKg , mUserHigh from u_colcokInfo  where 1=1 ;"];
            sqlite3_stmt *statement;
            if (sqlite3_prepare_v2(database, [selectStr UTF8String], -1, &statement, nil)==SQLITE_OK) {
                while (sqlite3_step(statement)==SQLITE_ROW) {
                    clockInfo *info=[self getInfo:statement];
                    [arr addObject:info];
                }
...
-(clockInfo*)queryInfo:(int)mid
{
    ...
    if (path!=Nil) {
        ...
        if(result==SQLITE_OK)
        {
            NSString* selectStr=[[NSString alloc]initWithFormat:@"select Id ,mUserCode ,mUserName ,mUserPwd ,mUserSex ,mUserAge ,mUserKg , mUserHigh from u_colcokInfo where Id=%d  ;",mid];
            sqlite3_stmt *statement;
            if (sqlite3_prepare_v2(database, [selectStr UTF8String], -1, &statement, nil)==SQLITE_OK) {
...
-(clockInfo*)queryInfoByName:(NSString*)mName{
    ...
    if (path!=Nil) {
        ...
        if(result==SQLITE_OK)
        {
            NSString* selectStr=[[NSString alloc]initWithFormat:@"select Id ,mUserCode ,mUserName ,mUserPwd ,mUserSex ,mUserAge ,mUserKg , mUserHigh from u_colcokInfo where mUserName='%@'  ;",mName];
            sqlite3_stmt *statement;
            if (sqlite3_prepare_v2(database, [selectStr UTF8String], -1, &statement, nil)==SQLITE_OK) {
                if (sqlite3_step(statement)==SQLITE_ROW) {
                    info=[self getInfo:statement];
...
-(int)getMaxID
{
    ...
    if (path!=Nil) {
        ...
        if(result==SQLITE_OK)
        {
            NSString* selectStr=[[NSString alloc]initWithFormat:@"select max(Id) from u_colcokInfo where Id>0  ;"];
            sqlite3_stmt *statement;
            if (sqlite3_prepare_v2(database, [selectStr UTF8String] , -1, &statement, nil)==SQLITE_OK) {
                if (sqlite3_step(statement)==SQLITE_ROW) {
